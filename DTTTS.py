...
def load_persona_data(persona_dir):
    """Load training persona data from text files (first 80% only, same logic as T-BoN.py)."""
    # Get and sort all persona files to ensure consistent ordering (same as T-BoN.py)
    persona_files = sorted(glob.glob(os.path.join(persona_dir, "pid_*_prompt.txt")))
    
    if not persona_files:
        print("Warning: No persona files found.")
        return {}

    # Split files into training and test sets based on index (same logic as T-BoN.py)
    split_index = int(len(persona_files) * 0.8)
    train_files = persona_files[:split_index]
    test_files = persona_files[split_index:]
    
    print(f"Loading {len(train_files)} training personas out of {len(persona_files)} total personas")
    print(f"Excluding {len(test_files)} personas for test set (same as T-BoN.py)")
...
def score_with_logprobs(persona_text, persona_id, ad_image, client, model_id):
    """
    Ask Gemini for effectiveness score with logprobs, returning comprehensive results.
    """
    # keep persona short like in your main prompt
...
def predict_ad_click_with_gemini(persona_text, persona_id, ad_image, client, model_id):
    ...
    try:
        ...
        result = score_with_logprobs(persona_text, persona_id, ad_image, client, model_id)
...
def run_dttts_simulation(personas, ad_images, client, model_id, scenario_name, num_timesteps=500, conf_target=0.95, check_every=20, k_post_samples=2000):
    ...
    # Convert personas dict to list for sampling
    ...
    results = {
        "simulation_metadata": {
            ...
            "num_ads": num_ads,
            "num_personas": len(persona_list),
            "algorithm": "Double TTTS (Alternating Best-Worst)"
        },
        "timestep_results": [],
        "final_statistics": {}
    }
    ...
    print(f"Available personas: {len(persona_list)}")
    print("-" * 50)
    
    for timestep in range(1, num_timesteps + 1):
        # Step 1: Sample a persona (with replacement)
        persona_id, persona_text = random.choice(persona_list)
        
        # Step 2: Use Double TTTS to select an ad (arm)
        selected_ad = dttts.select_arm(timestep)
        
        # Step 3: Get the ad image for the selected ad
        if ad_images[selected_ad] is None:
            print(f"Warning: Ad {selected_ad+1} image not available, skipping timestep {timestep}")
            continue
            
        # Step 4: Get prediction for this persona-ad combination
        ...
        # Step 5: Get effectiveness score (this is our multinomial outcome 1-5)
        effectiveness_score = prediction.get("effectiveness_score", 3)
        
        # Step 6: Update the TTTS algorithm
        ...
        if timestep <= 10 or timestep % 50 == 0:
            print(f"Step {timestep:4d} (Find {objective:5}): Persona {persona_id}, Ad {selected_ad+1}, Score {effectiveness_score}")
    ...
    # Calculate average effectiveness scores (instead of success rates)
...
def save_results(results, output_dir, scenario_name):
    ...
    output_file = os.path.join(output_dir, f"{scenario_name.lower()}_dttts_results.json")
    ...
    print(f"\nSaved DTTTS results to: {output_file}")
    
    # Print summary
    stats = results["final_statistics"]
    print("\n" + "="*60)
    print("DOUBLE TTTS SIMULATION SUMMARY")
    print("="*60)
    print(f"Total timesteps: {results['simulation_metadata']['num_timesteps']}")
...
def main():
    """Main execution function for DTTTS ad selection."""
    
    # Parse command line arguments
    parser = argparse.ArgumentParser(description='DTTTS Ad Selection Algorithm')
    ...
    NUM_TIMESTEPS = 5000  # Number of Double TTTS timesteps
    ...
    if test_mode:
        print("Running in TEST MODE - using all personas with reduced timesteps")
        NUM_TIMESTEPS = 5
        print(f"Test mode: {len(personas)} personas, {NUM_TIMESTEPS} timesteps")
    
    print("Loading ad images...")
    ad_images = load_ad_images(campaign_dir, SCENARIO_NAME)
    
    print("Running Double TTTS simulation...")
    results = run_dttts_simulation(personas, ad_images, client, model_id, SCENARIO_NAME, NUM_TIMESTEPS)
    
    print("Saving results...")
    save_results(results, output_dir, SCENARIO_NAME)
    
    print("Double TTTS simulation complete!")
